...
- Amp has no Cursor thinking slugs. `anthropic/claude-opus-5-5`, `openai/gpt-6-sol`, and `xai/grok-4.7` request high effort, while the standalone `grok47-xhigh` mode requests xhigh. Other raw model IDs have no effort override. `builtin:high` is GPT-6 Astra at medium effort, not GPT-6 Sol. Opus 5.5 is the balanced judgment default. `builtin:ultra` is not that model.
