# The amp-pstack guide

amp-pstack works best when you stop micromanaging the agent. You describe what you want and how you'll know it's done. The `poteto` mode, or `pstack:poteto-mode`, picks the playbook, runs the other skills as the steps need them, and shows you the evidence. This guide teaches that habit with realistic prompts.
...
```text
Use pstack:poteto-mode. The export writes duplicate rows when a retry lands mid-run. Repro first, then fix and verify.
```

You don't need to name a playbook or list skills. "Repro first" and a checkable outcome are all the routing signal poteto-mode needs. It matches the Bug fix playbook, copies the steps into a checklist, and calls the right skills as each step fires.
