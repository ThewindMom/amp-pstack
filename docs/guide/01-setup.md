...
The next local delegate resolves the changed map immediately. Reload plugins before using a changed model or effort in an orb because Amp publishes those custom agent definitions when the plugin loads.
