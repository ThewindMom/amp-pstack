...
- **Long-running work.** Playbooks use Amp child threads, schedules, and native capability webhooks instead of editor polling commands.
- **External wakeups.** Task-specific plugins use Amp's native `amp.createWebhook` API. They register stable keys from the plugin entry point, keep capability URLs secret, and make at-least-once effects durably idempotent.
...
Amp native webhooks are orb-only and at least once. A task-specific plugin must register its stable key from the entry point so reload reinstalls the handler, validate untrusted payloads, and persist `event.id` atomically with the business effect. Treat the returned URL as a bearer credential and keep it out of chat, commits, and logs.
...
`amp plugins list` shows the `setup-models` command, the five pstack tools (`start_agent`, `run_panel`, `stop_agent`, `read_current_thread`, and `configure_models`), the generated `pstack-*` orb adapter modes, and the `poteto` mode. It does not print `export const description`. That string is in `index.ts` and in `package.json`. Thread setup uses `pstack_configure_models` with `action: "profile"` or `pstack:setup-pstack`. The palette command is optional.
