...
- **Role-based agents.** `pstack_run_agent` maps roles to child Amp threads and waits for their reports. Cursor backgrounds every Task; Amp's unit is the thread. The tool always returns `threadID`. Timeout is `status: timeout` plus that ID, not a discarded spawn. Read the child. Do not redo its work in the parent. `pstack_start_agent` is the non-blocking form. Callers omit `timeoutMs`. The wait floor is ten minutes.
