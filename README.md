...
Configure model roles with the `pstack:setup-pstack` skill, the `pstack_configure_models` tool, or the `setup-models` command.
...
These launch targets apply to Amp-native workers. CLI-routed workers use a snapshot worktree on the current executor; omit `executor` and `launchTarget` to accept it. Explicit placement requests are rejected rather than silently redirected. See [CLI backends](#cli-backends).
...
Every Grok start checks `amp config model-providers list --json`. Native Grok is eligible only when the active SuperGrok connection precedes all other active connections that could serve `xai/grok-4.7`; otherwise it uses `cursor-agent` with `grok-4.7-xhigh-fast`. Grok Build is opt-in. Opus roles default to `claude -p --model opus`. Set `pstack_configure_models` with `action: "set", backends: { opus: "amp", grokBuild: false }` to choose native Opus. This changes pstack preferences, not provider settings or the coordinator's model.

CLI delegates have no Amp tools. Each receives a fresh worktree containing tracked edits and non-ignored untracked files. Read-only delegates use the CLI read-only mode and a post-run filesystem comparison; their `node_modules` symlink targets a private dependency copy. Writers install dependencies as directed by their brief and return a binary patch for parent review. The plugin never applies that patch automatically.

CLI IDs start with `cli-`, are not Amp thread IDs, and cannot be steered with native messaging. The plugin delivers their final text and patch path. `pstack_stop_agent` accepts these IDs. Orb services keep runs alive across plugin restarts; a stopped service without a result becomes an error. Results are pruned seven days after completion. Every run uses one backend. Startup failures and mid-task limits are reported, never replayed or automatically sent to another backend. Reconcile the terminal result before explicitly starting replacement work.
