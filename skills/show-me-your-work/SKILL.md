...
Use the helper `scripts/log.sh <logfile> <phase> <decision> <why> <evidence> <result>`. It stamps `ts`, appends the header when the file is missing or empty, strips stray tabs/newlines, and prefixes any cell starting with `=`, `+`, `-`, or `@` with a single quote. The header write uses `>>`, never `>`, so a failed existence check cannot erase existing rows. A bare `printf` appending a row works too, but mind those same bytes if cells come from generated or user-supplied text.
...
At the end of the run, before handing back, check the log told the truth. Read this Amp thread with `pstack_read_current_thread`; when the work ran in child threads, read those specific threads with `read_thread`. Do not search unrelated threads. Walk the log against what actually happened:
