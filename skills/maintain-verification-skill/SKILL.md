...
description: "Only use when named or routed by poteto-mode. Audits a project's verification skill and feature map with parallel source readers and one live session when asked to maintain or audit the verify skill."
---
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.agents/skills/verify-*/`). Several candidates → ask which one; none → stop and load **create-verification-skill** instead of inventing a target.
...
2. **Source wave.** One read-only child per feature file, launched concurrently with `pstack_start_agent`, role `how-explorer`. Follow `../poteto-mode/references/amp-adapter.md`. Do not use Amp builtin `Task`. Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe. Children never drive the app and never edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe.
