...
1. Call `pstack_run_agent` with role `comment-reviewer`, `executor: "local"`, and no `timeoutMs`. Pass the checkout or git refs plus the path `../../agents/comment-sicko.md`. Do not paste the rules file. Do not ask it to edit. Keep the `threadID`. On timeout, read that child. Do not run the comment audit in the parent. The plugin floors the wait at ten minutes and gives the reviewer a strict read-only allowlist: no shell, file writes, spawn tools, MCP, or unknown plugin tools.
