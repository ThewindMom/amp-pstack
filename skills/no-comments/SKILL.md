...
1. Call `pstack_run_agent` with role `comment-reviewer`, local executor, and no `timeoutMs`. Pass the checkout or git refs plus the path `../../agents/comment-sicko.md`. Do not paste the rules file. Do not ask it to edit. Keep the `threadID`. On timeout, read that child. Do not run the comment audit in the parent. The plugin floors the wait at ten minutes, keeps read-only `shell_command`, and strips write tools, `skill`, `Task`, and pstack spawn tools so the reviewer cannot recurse.
