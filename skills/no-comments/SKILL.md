...
1. Read `../../agents/comment-sicko.md` and pass its body plus the scope to `pstack_run_agent` with role `comment-reviewer`, local executor. The plugin strips write tools, `shell_command`, `shell_command_kill`, and `Task` from that role. Do not ask it to edit.
