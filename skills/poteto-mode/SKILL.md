...
**Files.** Threads do not share a filesystem. A steer message does not move files. Only a same-machine local parent and local child share the checkout; cite paths, do not copy. If either thread is an orb (parent orb, child orb, or both), treat them as isolated machines, same as local-parent / orb-child. Transfer uncommitted fixtures, screenshots, dumps, store files, and anything the brief cannot point at in git. Parent → child: `upload_thread_file` (4 MiB, destination parent dir must exist). Child → parent: child writes, cites the path, parent `download_thread_file`. Need a URL: `thread_file_url` (expires). Two orbs do not share a disk with each other. Do not paste a file body into a brief when a transfer can carry it. Do not push only to make an orb see a file unless the user authorized that push.
