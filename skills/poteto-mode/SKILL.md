...
Model selection is role-based and configurable through the **setup-pstack** skill, `pstack_configure_models`, `.amp/pstack.models.json`, or `~/.config/amp/pstack.models.json`. Specified code, prose, and judgment default to `anthropic/claude-fable-5-1`. Mechanical work defaults to `xai/grok-4.6`. Panels are Fable 5.1, Sol, Grok, and Opus 5. Copy the example JSON or apply profile `cheap` to drop Fable and Opus. Feature and refactoring share role `feature-refactoring`; the aliases `feature` and `refactoring` resolve to it. A configured `builtin:low`, `builtin:medium`, `builtin:high`, or `builtin:ultra` still runs as a pstack delegate: the plugin extends that Amp mode and keeps the pstack instructions.
