...
const PROGRAM_MARKERS = ["durable objective", "loaded skill", /30[- ]minute/, "status message"];
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.test(live.rest)) fail(live.n, `${pr.title}: Verify, live lacks "Ten lanes on \`<swarm-worker model>\` at the PR head" with the model filled in`);
