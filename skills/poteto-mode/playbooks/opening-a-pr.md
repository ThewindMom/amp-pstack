...
**Readiness.** Open every PR ready, never as a draft, unless the user explicitly asks for a draft. Set `draft: false` on creation. If it still opens as a draft, run `gh pr ready <number>`. Run `gh pr view <number>` before referring to PR status.

**Babysit.** Opening a PR does not start a babysit. Post the URL and keep building. Finish the phase or stack first. Run a separate babysit pass only when the user asks for one after the whole stack exists. A babysit for each new PR stalls the build and spends checks on commits that later waves restart. Push back when feedback drifts from intent.

An agent that opens a PR runs **interrogate**, the diff cleanup above, and **no-comments**. It reports the URL to the parent thread and does not babysit unless its brief says so.
