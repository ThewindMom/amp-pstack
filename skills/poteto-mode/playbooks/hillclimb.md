...
5. Loop, one hypothesis per iteration:
   - Hand the change to `pstack_start_agent` with role `hillclimb` and a tight non-empty `scope`. Follow `references/amp-adapter.md`. Supervise and review the diff rather than typing it (the **guard-the-context-window** principle skill). When several independent hypotheses are live, fan them to parallel agents, each in its own worktree (the **separate-before-serializing-shared-state** principle skill).
