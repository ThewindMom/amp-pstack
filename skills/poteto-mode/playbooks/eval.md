...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's Amp child thread with `read_thread`. Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
