...
6. **Verify the chain from thread transcripts, not self-report.** Read each candidate's Amp child thread with `read_thread`. Look at which files and tools it actually used. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the thread plus the shape of the code, never from the candidate's own claims.
