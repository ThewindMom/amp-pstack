...
1. Stop at a safe boundary. Finish the current atomic step or back out of it. Never stop mid-edit in a known-broken state. Start nothing new. Send active child threads a zero-writes hold, then cancel tracked children with `pstack_stop_agent` and confirm terminal state. A hold message alone is not a stopped worker.
