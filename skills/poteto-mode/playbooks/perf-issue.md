...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_start_agent`, role `perf-issue`, local executor when the checkout matters. Keep the `threadID`. Join on the child's report. Do not call `pstack_run_agent` for this step. Do not apply the same fix in the parent. The plugin resolves the configured model (default `anthropic/claude-fable-5-1`); review the diff. Capture a post-fix trace.
