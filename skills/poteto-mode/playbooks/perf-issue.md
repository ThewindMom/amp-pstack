...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_start_agent`, role `perf-issue`, and a non-empty `scope`. An orb parent defaults to `parent-project-orb`. From a local parent, use `current-checkout` when the trace or workload depends on local state. Use `parent-project-orb` for clean work from the project remote and `native-orb` when project, size, or mode matters. Keep dependent work in a parent orb or transfer its files before spawning. Keep the `threadID`. Join on the child's report. Do not call `pstack_run_agent` for this step. Do not apply the same fix in the parent. The plugin resolves the configured model (default `xai/grok-4.6`); review the diff. Capture a post-fix trace.
