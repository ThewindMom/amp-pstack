...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_run_agent`, role `perf-issue`, local executor when the checkout matters. The plugin resolves the configured model (default `openai/gpt-5.6-sol`); review the diff. Capture a post-fix trace.
