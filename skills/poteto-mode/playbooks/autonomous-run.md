...
2. Pick the Amp wake mechanism. For independent execution, spawn an orb child thread with `pstack_start_agent` or Amp's `create_thread`; use `named-runner` for a specifically required machine. For an event from another service, create an orb-owned capability webhook with `pstack_create_wake_webhook`. Amp shows the URL through UI; do not paste it into the transcript. Webhook delivery is at least once, so validate the event domain and make the action idempotent. Registration and receipts recover on the same persistent executor, not after loss of that executor's disk. For periodic or delayed checks, set an Amp schedule only when the user requested ongoing or later work. When the condition should resolve in minutes, wait in the current turn instead of scheduling.
