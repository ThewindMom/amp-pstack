...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_start_agent`, role `bug-fix`, and a non-empty `scope`. Follow `references/amp-adapter.md`. Give a specific scope.
