...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_run_agent`, role `bug-fix`, local executor when the checkout matters. The plugin resolves the configured model (default `anthropic/claude-fable-5-1`). Give a specific scope; review the diff.
