...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_start_agent`, role `bug-fix`, local executor when the checkout matters. Keep the `threadID`. End the turn or keep doing parent work. Join on the child's `pstack_send_to_thread` report. Do not call `pstack_run_agent` for this step. Do not apply the same fix in the parent. The plugin resolves the configured model (default `anthropic/claude-fable-5-1`). Give a specific scope; review the diff.
