...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_start_agent`, role `bug-fix`, and a non-empty `scope`. Follow `references/amp-adapter.md`. Give a specific scope and inline the complete plan. Name and explicitly require only applicable execution skills. Select **tdd** only when the bug has the cheap local test path described in step 5; an expensive, integration-heavy, or unclear test path does not automatically select it. Select a language or surface skill only when the assigned files and verification surface match it.
