...
Role-based, configurable through **setup-pstack**, `pstack_configure_models`, plugin `pstack.models.json`, `.amp/pstack.models.json`, or `~/.config/amp/pstack.models.json`. Code delegates default to `xai/grok-4.7`. Hardest changes, prose, and judgment default to `anthropic/claude-opus-5-5`. Panels are Opus 5.5, Sol, and Grok. The bundled plugin JSON may drop Fable and Opus (cheap). Feature and refactoring are independently configurable. A configured `builtin:low`, `builtin:medium`, `builtin:high`, or `builtin:ultra` still runs as a pstack delegate. Raw Grok 4.7 pstack delegates request `reasoningEffort: medium`; the standalone `grok47-xhigh` mode remains xhigh. `builtin:*` are Amp modes, not Cursor thinking slugs.

The `poteto` parent is Amp builtin `medium` plus **poteto-mode**, pinned to `openai/gpt-6-sol` at medium reasoning. It does not copy ultra tools. The official `gpt6s` mode is GPT-6 Sol at high effort and is not this parent. Grok stays on code and explorer workers. Do not start pstack work in official `grok46`. Never call `public_artifact_url` except for an image or video the user asked to share. Never call `painter` unless the user asked for an image.
