---
name: automate-me
description: "Only use when named or routed by poteto-mode. Drafts or revises a personal -mode skill via create-skill and unslop for requests to capture or automate the user's working style."
builtin-tools:
  - find_thread
  - read_thread
  - pstack_run_agent
...
Use `find_thread` with `author:me` to search the active project or repository and the requested date range. Read relevant conversations with `read_thread`. For a large result set, run parallel `pstack_start_agent` calls with role `how-explorer` over non-overlapping thread-ID slices. Follow `../poteto-mode/references/amp-adapter.md`. Each miner returns recurring patterns with Amp thread links. Default signals worth hunting:
