...
Launch all workers concurrently with `pstack_start_agent`, role `swarm-worker`. Route from the parent executor first. An orb parent defaults workers to `parent-project-orb`. From a local parent, use `current-checkout` when workers need local or uncommitted state and `parent-project-orb` for clean work from the project remote. Use `repo-independent-orb` only for work that needs no checkout. Use `native-orb` when project, size, or mode matters. Keep every `threadID`. Each child exclusively owns its slice. Continue independent parent work, then end the turn when the reports block further progress. Join on `pstack_send_to_thread` reports. Relay sibling findings through the parent; workers do not message each other. If this parent or a worker is an orb and the worker needs an uncommitted fixture, `upload_thread_file` it. Two orbs do not share a disk. Do not freeze the parent on `pstack_run_agent`. Never redo or replace a live child's slice. A missing report is a dropout only after you read that child and confirm it has none.

Orb size follows the **poteto-mode** Agents and threads table. Coverage slices that only read and report use `create_thread` with `a1.tiny` or `a1.small`. Live visual lanes, browsers, or CPU-heavy tests use `a1.large`. `parent-project-orb` uses the parent project's default size. `repo-independent-orb` is enough when the brief needs no checkout. A user-named size always wins.
